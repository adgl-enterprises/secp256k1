...
secp256k1_bulletproof_generators *secp256k1_bulletproof_generators_create(const secp256k1_context *ctx, const secp256k1_generator *blinding_gen, size_t n) {
    ...
    secp256k1_zkp_rfc6979_hmac_sha256_initialize(ctx, &rng, seed, 64);
    for (i = 0; i < n; i++) {
        unsigned char tmp[32] = { 0 };
        secp256k1_generator gen;
        secp256k1_zkp_rfc6979_hmac_sha256_generate(ctx, &rng, tmp, 32);
...
int secp256k1_bulletproof_rangeproof_verify(const secp256k1_context* ctx, secp256k1_scratch *scratch, const secp256k1_bulletproof_generators *gens, const unsigned char *proof, size_t plen,
 const uint64_t *min_value, const secp256k1_pedersen_commitment* commit, size_t n_commits, size_t nbits, const secp256k1_generator *value_gen, const unsigned char *extra_commit, size_t extra_commit_len) {
    ...
    if (!secp256k1_zkp_scratch_allocate_frame(ctx, scratch, 2 * n_commits * sizeof(secp256k1_ge), 1, &_zkp_scratch_cp)) {
        return 0;
    }

    commitp = (secp256k1_ge *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_commits * sizeof(secp256k1_ge));
    if (commitp == NULL) {
    ...
    ret = secp256k1_bulletproof_rangeproof_verify_impl(ctx, scratch, &proof, 1, plen, nbits, &minvalue_ptr, &commitp_ptr, n_commits, &value_genp, gens, &extra_commit, &extra_commit_len);
    secp256k1_zkp_scratch_frame_end(ctx, scratch, _zkp_scratch_cp);
    return ret;
}

int secp256k1_bulletproof_rangeproof_verify_multi(const secp256k1_context* ctx, secp256k1_scratch *scratch, const secp256k1_bulletproof_generators *gens, const unsigned char* const* proof, size_t n_proofs, size_t plen, const uint64_t* const* min_value, const secp256k1_pedersen_commitment* const* commit, size_t n_commits, size_t nbits, const secp256k1_generator *value_gen, const unsigned char* const* extra_commit, size_t *extra_commit_len) {
    int ret;
    secp256k1_ge **commitp;
    secp256k1_ge *value_genp;
    size_t i;
    ...
    if (!secp256k1_zkp_scratch_allocate_frame(ctx, scratch, n_proofs * (sizeof(*value_genp) + sizeof(*commitp) + n_commits * sizeof(**commitp)), 2 + n_proofs, &_zkp_scratch_cp)) {
        return 0;
    }

    commitp = (secp256k1_ge **)secp256k1_zkp_scratch_alloc(ctx, scratch, n_proofs * sizeof(*commitp));
    value_genp = (secp256k1_ge *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_proofs * sizeof(*value_genp));
    ...
    for (i = 0; i < n_proofs; i++) {
        size_t j;
        commitp[i] = (secp256k1_ge *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_commits * sizeof(*commitp[i]));
        if (commitp[i] == NULL) {
            secp256k1_zkp_scratch_frame_end(ctx, scratch, _zkp_scratch_cp);
            return 0;
        }
    ...
    ret = secp256k1_bulletproof_rangeproof_verify_impl(ctx, scratch, proof, n_proofs, plen, nbits, min_value, (const secp256k1_ge **) commitp, n_commits, value_genp, gens, extra_commit, extra_commit_len);
    secp256k1_zkp_scratch_frame_end(ctx, scratch, _zkp_scratch_cp);
    return ret;
}
...
int secp256k1_bulletproof_rangeproof_prove(
    ...
    {
        size_t frame_size = n_commits * (sizeof(*commitp) + sizeof(*blinds));
        size_t n_objects = 2;
        if (t_one != NULL) {
            frame_size += 2 * sizeof(secp256k1_ge);
            n_objects = 3;
        }
        if (!secp256k1_zkp_scratch_allocate_frame(ctx, scratch, frame_size, n_objects, &_zkp_scratch_cp)) {
            return 0;
        }
    }
    commitp = (secp256k1_ge *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_commits * sizeof(*commitp));
    blinds = (secp256k1_scalar *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_commits * sizeof(*blinds));
    ...
    if (t_one != NULL) {
        tge = (secp256k1_ge *)secp256k1_zkp_scratch_alloc(ctx, scratch, 2 * sizeof(secp256k1_ge));
        if (tge == NULL) {
            secp256k1_zkp_scratch_frame_end(ctx, scratch, _zkp_scratch_cp);
            return 0;
        }
