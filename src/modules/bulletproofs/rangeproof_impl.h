...
static int secp256k1_bulletproof_rangeproof_verify_impl(const secp256k1_context *ctx, secp256k1_scratch *scratch, const unsigned char* const* proof, const size_t n_proofs, const size_t plen, size_t nbits, const uint64_t* const* min_value, const secp256k1_ge* const* commitp, size_t n_commits, const secp256k1_ge *value_gen, const secp256k1_bulletproof_generators *gens, const unsigned char* const* extra_commit, size_t *extra_commit_len) {
    ...
    if (!secp256k1_zkp_scratch_allocate_frame(ctx, scratch, n_proofs * (sizeof(*ecmult_data) + sizeof(*innp_ctx)), 2, &_zkp_scratch_cp)) {
        return 0;
    }
    ecmult_data = (secp256k1_bulletproof_vfy_ecmult_context *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_proofs * sizeof(*ecmult_data));
    innp_ctx = (secp256k1_bulletproof_innerproduct_context *)secp256k1_zkp_scratch_alloc(ctx, scratch, n_proofs * sizeof(*innp_ctx));
    ...
    for (i = 0; i < n_proofs; i++) {
        ...
        if (min_value != NULL && min_value[i] != NULL) {
            ...
            len[2] = n_commits >> 16;
            len[3] = n_commits >> 24;
            secp256k1_zkp_sha256_write(ctx, &sha256, len, 4);
            for (j = 0; j < n_commits; j++) {
                ...
                secp256k1_zkp_sha256_write(ctx, &sha256, vbuf, 8);
            }
            secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
        }
        for (j = 0; j < n_commits; j++) {
            secp256k1_bulletproof_update_commit(ctx, commit, &commitp[i][j], &value_gen[i]);
        }
        if (extra_commit != NULL && extra_commit[i] != NULL) {
            secp256k1_zkp_sha256_initialize(ctx, &sha256);
            secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
            secp256k1_zkp_sha256_write(ctx, &sha256, extra_commit[i], extra_commit_len[i]);
            secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
        }
        ...
        secp256k1_bulletproof_update_commit(ctx, commit, &age, &sge);
        ...
        secp256k1_bulletproof_update_commit(ctx, commit, &ecmult_data[i].t1, &ecmult_data[i].t2);
        ...
        secp256k1_zkp_sha256_initialize(ctx, &sha256);
        secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
        secp256k1_zkp_sha256_write(ctx, &sha256, &proof[i][0], 64);
        secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);

        secp256k1_zkp_sha256_initialize(ctx, &sha256);
        secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
        secp256k1_zkp_sha256_finalize(ctx, &sha256, randomizer61);
    ...
    ret = secp256k1_bulletproof_inner_product_verify_impl(ctx, scratch, gens, nbits * n_commits, innp_ctx, n_proofs, plen - (64 + 128 + 1), same_generators);
    secp256k1_zkp_scratch_frame_end(ctx, scratch, _zkp_scratch_cp);
    return ret;
}
...
static int secp256k1_bulletproof_rangeproof_prove_impl(
    ...
    for (i = 0; i < n_commits; i++) {
        secp256k1_bulletproof_update_commit(ctx, commit, &commitp[i], value_gen); /* TODO be less stupid about this */
    }
    if (extra_commit != NULL) {
        secp256k1_zkp_sha256_initialize(ctx, &sha256);
        secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
        secp256k1_zkp_sha256_write(ctx, &sha256, extra_commit, extra_commit_len);
        secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
    }
    ...
    secp256k1_bulletproof_update_commit(ctx, commit, &out_pt[0], &out_pt[1]);
    secp256k1_scalar_set_b32(&y, commit, &overflow);
    if (overflow || secp256k1_scalar_is_zero(&y)) {
        return 0;
    }
    secp256k1_bulletproof_update_commit(ctx, commit, &out_pt[0], &out_pt[1]); /* TODO rehashing A and S to get a second challenge is overkill */
    ...
    secp256k1_bulletproof_update_commit(ctx, commit, &out_pt[2], &out_pt[3]);
    ...
    secp256k1_zkp_sha256_initialize(ctx, &sha256);
    secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
    secp256k1_zkp_sha256_write(ctx, &sha256, proof, 64);
    secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
    ...
    *plen -= 64 + 128 + 1;
    ...
    if (secp256k1_bulletproof_inner_product_prove_impl(ctx, scratch, &proof[64 + 128 + 1], plen, gens, &y, nbits * n_commits, secp256k1_bulletproof_abgh_callback, (void *) &abgh_data, commit) == 0) {
        return 0;
    }
    *plen += 64 + 128 + 1;
...
static int secp256k1_bulletproof_rangeproof_rewind_impl(const secp256k1_context *ctx, uint64_t *value, secp256k1_scalar *blind, const unsigned char *proof, const size_t plen, uint64_t min_value, const secp256k1_pedersen_commitment *pcommit, const secp256k1_generator *value_gen, const unsigned char *nonce, const unsigned char *extra_commit, size_t extra_commit_len, unsigned char *message) {
    ...
    secp256k1_bulletproof_update_commit(ctx, commit, &commitp, &value_genp);
    ...
    secp256k1_zkp_sha256_write(ctx, &sha256, &proof[65], 64);
    secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
    ...
    secp256k1_zkp_sha256_write(ctx, &sha256, commit, 32);
    secp256k1_zkp_sha256_write(ctx, &sha256, &lrparity, 1);
    secp256k1_zkp_sha256_write(ctx, &sha256, &proof[129], 64);
    secp256k1_zkp_sha256_finalize(ctx, &sha256, commit);
