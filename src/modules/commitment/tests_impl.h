...
static void test_commitment_api(void) {
    ...
    CHECK(secp256k1_memcmp_var(out, out2, 33) == 0);
    /* Value: 1 and 2*/
    random_scalar_order_test(&tmp_s);
    secp256k1_scalar_get_b32(blind, &tmp_s);
    memset(blind_out, 0, 32);
...
void test_multiple_generators(void) {
    ...
    for (i = 0; i < n_generators; i++) {
        generator_blind[i] = (unsigned char*) malloc(sizeof(generator_seed));
        pedersen_blind[i] = (unsigned char*) malloc(sizeof(generator_seed));
...
void test_switch(void) {
    ...
    uint64_t val = secp256k1_rand32();
    /* Compressed encoding is portable; pubkey.data is not (ge_to_bytes is platform-dependent). */
    ...
    CHECK(secp256k1_ec_pubkey_parse(local_ctx, &switch_pubkey, GENERATOR_J_COMPR, 33));
    secp256k1_rand256(blind);
    CHECK(secp256k1_blind_switch(local_ctx, blind_switch, blind, val, &secp256k1_generator_const_h, &secp256k1_generator_const_g, &switch_pubkey));
    CHECK(secp256k1_memcmp_var(blind_switch, blind, 32) != 0);
    CHECK(secp256k1_blind_switch(local_ctx, blind_switch_2, blind, val, &secp256k1_generator_const_h, &secp256k1_generator_const_g, &switch_pubkey));
    CHECK(secp256k1_memcmp_var(blind_switch_2, blind_switch, 32) == 0);
    secp256k1_context_destroy(local_ctx);
}

REPEAT_TEST_MULT(test_pedersen, 10)

static const struct tf_test_entry tests_commitment[] = {
    CASE1(test_commitment_api),
    CASE1(test_pedersen),
