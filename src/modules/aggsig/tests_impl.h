...
#ifndef SECP256K1_MODULE_AGGSIG_TESTS
#define SECP256K1_MODULE_AGGSIG_TESTS
...
void test_aggsig_api(void) {
    ...
    secp256k1_scratch *scratch = secp256k1_scratch_create(&none->error_callback, 1024*4096);
    ...
    /* Upstream always builds ecmult_gen; any context may generate nonces. */
    CHECK(secp256k1_aggsig_generate_nonce(sign, aggctx, 0));
    CHECK(ecount == 2);
    CHECK(secp256k1_aggsig_generate_nonce(vrfy, aggctx, 1));
    CHECK(ecount == 2);
    CHECK(secp256k1_aggsig_generate_nonce(none, aggctx, 2));
    CHECK(ecount == 2);
    ...
    CHECK(ecount == 4);
    CHECK(secp256k1_aggsig_partial_sign(vrfy, aggctx, &partials[2], msg, seckeys[2], 2));
    CHECK(ecount == 4);
    CHECK(secp256k1_aggsig_partial_sign(sign, aggctx, &partials[3], msg, seckeys[3], 3));
    CHECK(ecount == 4);
...
static const struct tf_test_entry tests_aggsig[] = {
    CASE1(test_aggsig_api),
    CASE1(test_aggsig_onesigner),
};
