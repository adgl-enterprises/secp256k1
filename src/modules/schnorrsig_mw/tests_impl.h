...
#ifndef SECP256K1_MODULE_SCHNORRSIG_MW_TESTS_H
#define SECP256K1_MODULE_SCHNORRSIG_MW_TESTS_H
...
void test_schnorrsig_mw_serialize(void) {
    secp256k1_schnorrsig_mw sig;
    unsigned char in[64];
    unsigned char out[64];

    memset(in, 0x12, 64);
    CHECK(secp256k1_schnorrsig_mw_parse(CTX, &sig, in));
    CHECK(secp256k1_schnorrsig_mw_serialize(CTX, out, &sig));
    CHECK(secp256k1_memcmp_var(in, out, 64) == 0);
}

void test_schnorrsig_mw_api(secp256k1_scratch *scratch) {
    ...
    secp256k1_schnorrsig_mw sig;
    const secp256k1_schnorrsig_mw *sigptr = &sig;
    ...
    /** main test body **/
    ecount = 0;
    CHECK(secp256k1_schnorrsig_mw_sign(none, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_sign(vrfy, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_sign(sign, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_sign(sign, NULL, &nonce_is_negated, msg, sk1, NULL, NULL) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_schnorrsig_mw_sign(sign, &sig, NULL, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 1);
    CHECK(secp256k1_schnorrsig_mw_sign(sign, &sig, &nonce_is_negated, NULL, sk1, NULL, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_schnorrsig_mw_sign(sign, &sig, &nonce_is_negated, msg, NULL, NULL, NULL) == 0);
    CHECK(ecount == 3);

    ecount = 0;
    CHECK(secp256k1_schnorrsig_mw_serialize(none, sig64, &sig) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_serialize(none, NULL, &sig) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_schnorrsig_mw_serialize(none, sig64, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_schnorrsig_mw_parse(none, &sig, sig64) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_schnorrsig_mw_parse(none, NULL, sig64) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_schnorrsig_mw_parse(none, &sig, NULL) == 0);
    CHECK(ecount == 4);

    ecount = 0;
    CHECK(secp256k1_schnorrsig_mw_verify_batch(none, scratch, &sigptr, &msgptr, &pkptr, 1) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(sign, scratch, &sigptr, &msgptr, &pkptr, 1) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(vrfy, scratch, &sigptr, &msgptr, &pkptr, 1) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(vrfy, scratch, NULL, NULL, NULL, 0) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(vrfy, scratch, NULL, &msgptr, &pkptr, 1) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(vrfy, scratch, &sigptr, NULL, &pkptr, 1) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(vrfy, scratch, &sigptr, &msgptr, NULL, 1) == 0);
    CHECK(ecount == 3);
...
void test_schnorrsig_mw_bip_vectors_check_signing(secp256k1_scratch *scratch, const unsigned char *sk, const unsigned char *pk_serialized, const unsigned char *msg, const unsigned char *expected_sig, const int expected_nonce_is_negated) {
    secp256k1_schnorrsig_mw sig;
    secp256k1_pubkey pk;
    const secp256k1_schnorrsig_mw *sigptr = &sig;
    const unsigned char *msgptr = msg;
    const secp256k1_pubkey *pkptr = &pk;

    (void)expected_sig;
    (void)expected_nonce_is_negated;

    CHECK(secp256k1_ec_pubkey_parse(CTX, &pk, pk_serialized, 33) == 1);
    CHECK(secp256k1_schnorrsig_mw_sign(CTX, &sig, NULL, msg, sk, NULL, NULL) == 1);
    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, &sigptr, &msgptr, &pkptr, 1) == 1);
}

/* Helper function for schnorrsig_bip_vectors
 * Checks that both verify and verify_batch return the same value as expected. */
void test_schnorrsig_mw_bip_vectors_check_verify(secp256k1_scratch *scratch, const unsigned char *pk_serialized, const unsigned char *msg32, const unsigned char *sig_serialized, int expected) {
    secp256k1_pubkey pk;
    secp256k1_schnorrsig_mw sig;
    const secp256k1_schnorrsig_mw *sig_arr[1];
    const unsigned char *msg_arr[1];
    const secp256k1_pubkey *pk_arr[1];

    CHECK(secp256k1_ec_pubkey_parse(CTX, &pk, pk_serialized, 33) == 1);
    CHECK(secp256k1_schnorrsig_mw_parse(CTX, &sig, sig_serialized) == 1);
    sig_arr[0] = &sig;
    msg_arr[0] = msg32;
    pk_arr[0] = &pk;
    CHECK(expected == secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, sig_arr, msg_arr, pk_arr, 1));
}
 ...
 */
void test_schnorrsig_mw_bip_vectors(secp256k1_scratch *scratch) {
    {
        /* Test vector 1 */
        ...
        test_schnorrsig_mw_bip_vectors_check_signing(scratch, sk1, pk1, msg1, sig1, 1);
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk1, msg1, sig1, 1);
    }
    {
        /* Test vector 2 */
        ...
        test_schnorrsig_mw_bip_vectors_check_signing(scratch, sk2, pk2, msg2, sig2, 0);
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk2, msg2, sig2, 1);
    }
    {
        /* Test vector 3 */
        ...
        test_schnorrsig_mw_bip_vectors_check_signing(scratch, sk3, pk3, msg3, sig3, 0);
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk3, msg3, sig3, 1);
    }
    {
        /* Test vector 4 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk4, msg4, sig4, 1);
    }
    {
        /* Test vector 5 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk5, msg5, sig5, 1);
    }
    {
        /* Test vector 6 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk6, msg6, sig6, 1);
    }
    {
        /* Test vector 7 */
    ...
    }
    {
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk8, msg8, sig8, 0);
    }
    {
        /* Test vector 9 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk9, msg9, sig9, 0);
    }
    {
        /* Test vector 10 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk10, msg10, sig10, 0);
    }
    {
        /* Test vector 11 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk11, msg11, sig11, 0);
    }
    {
        /* Test vector 12 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk12, msg12, sig12, 0);
    }
    {
        /* Test vector 13 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk13, msg13, sig13, 0);
    }
    {
        /* Test vector 14 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk14, msg14, sig14, 0);
    }
    {
        /* Test vector 15 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk15, msg15, sig15, 0);
    }
    {
        /* Test vector 16 */
        ...
        test_schnorrsig_mw_bip_vectors_check_verify(scratch, pk16, msg16, sig16, 0);
    }
}

/* Nonce function that returns constant 0 */
static int mw_nonce_function_failing(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
    (void) msg32;
    (void) key32;
    (void) algo16;
    (void) data;
...
static int mw_nonce_function_0(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
    (void) msg32;
    (void) key32;
    (void) algo16;
    (void) data;
...
void test_schnorrsig_mw_sign(void) {
    unsigned char sk[32];
    unsigned char msg[32];
    secp256k1_schnorrsig_mw sig;

    memcpy(msg, "this is a msg for a schnorrsig..", sizeof(msg));
    ...
    CHECK(secp256k1_schnorrsig_mw_sign(CTX, &sig, NULL, msg, sk, NULL, NULL) == 0);
    memset(sk, 23, sizeof(sk));

    CHECK(secp256k1_schnorrsig_mw_sign(CTX, &sig, NULL, msg, sk, mw_nonce_function_failing, NULL) == 0);
    CHECK(secp256k1_schnorrsig_mw_sign(CTX, &sig, NULL, msg, sk, mw_nonce_function_0, NULL) == 0);
}
...
void test_schnorrsig_mw_sign_verify(secp256k1_scratch *scratch) {
    unsigned char sk[32];
    unsigned char msg[N_SIGS][32];
    secp256k1_schnorrsig_mw sig[N_SIGS];
    size_t i;
    const secp256k1_schnorrsig_mw *sig_arr[N_SIGS];
    const unsigned char *msg_arr[N_SIGS];
    const secp256k1_pubkey *pk_arr[N_SIGS];
    secp256k1_pubkey pk;

    memcpy(sk, "shhhhhhhh! this key is a secret.", sizeof(sk));
    CHECK(secp256k1_ec_pubkey_create(CTX, &pk, sk));

    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, NULL, NULL, NULL, 0));

    for (i = 0; i < N_SIGS; i++) {
        secp256k1_rand256(msg[i]);
        CHECK(secp256k1_schnorrsig_mw_sign(CTX, &sig[i], NULL, msg[i], sk, NULL, NULL));
        sig_arr[i] = &sig[i];
        msg_arr[i] = msg[i];
        pk_arr[i] = &pk;
    }

    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, sig_arr, msg_arr, pk_arr, 1));
    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, sig_arr, msg_arr, pk_arr, 2));
    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, sig_arr, msg_arr, pk_arr, 4));
    CHECK(secp256k1_schnorrsig_mw_verify_batch(CTX, scratch, sig_arr, msg_arr, pk_arr, N_SIGS));
...
static void test_schnorrsig_mw_api_wrapper(void) {
    secp256k1_scratch *scratch = secp256k1_scratch_create(&CTX->error_callback, 1024 * 1024);
    test_schnorrsig_mw_api(scratch);
    secp256k1_scratch_destroy(&CTX->error_callback, scratch);
}

static void test_schnorrsig_mw_bip_vectors_wrapper(void) {
    secp256k1_scratch *scratch = secp256k1_scratch_create(&CTX->error_callback, 1024 * 1024);
    test_schnorrsig_mw_bip_vectors(scratch);
    secp256k1_scratch_destroy(&CTX->error_callback, scratch);
}

static void test_schnorrsig_mw_sign_verify_wrapper(void) {
    secp256k1_scratch *scratch = secp256k1_scratch_create(&CTX->error_callback, 1024 * 1024);
    test_schnorrsig_mw_sign_verify(scratch);
    secp256k1_scratch_destroy(&CTX->error_callback, scratch);
}

static const struct tf_test_entry tests_schnorrsig_mw[] = {
    CASE1(test_schnorrsig_mw_serialize),
    CASE1(test_schnorrsig_mw_api_wrapper),
    CASE1(test_schnorrsig_mw_bip_vectors_wrapper),
    CASE1(test_schnorrsig_mw_sign),
    CASE1(test_schnorrsig_mw_sign_verify_wrapper),
};
